...
試験対策・学習用Webアプリケーション。問題集の作成・管理、演習/実戦モードでの出題、詳細統計による弱点分析を提供する。
...
- 科目マスタによる分類
- 問題集・問題・選択肢のCRUD（管理画面）
- タグによる柔軟な分類（問題集・問題の両レベル）
- JSON インポート/エクスポート
...
| リソース | エンドポイント | 概要 |
|---------|--------------|------|
| Subjects | `GET/POST /subjects`, `GET/PUT/DELETE /subjects/:id` | 科目 CRUD |
| Tags | `GET/POST /tags`, `PUT/DELETE /tags/:id` | タグ CRUD |
| Workbooks | `GET/POST /workbooks`, `GET/PUT/DELETE /workbooks/:id` | 問題集 CRUD |
| Questions | `POST/PUT/DELETE /workbooks/:setId/questions/:id` | 問題 CRUD |
| Import/Export | `POST /workbooks/import`, `GET /workbooks/:id/export` | JSON 入出力 |
| Sessions | `POST /sessions`, `GET /:id/questions/:index`, `POST /:id/answers` | 出題エンジン |
| Stats | `GET /stats/overview\|subjects\|tags\|history\|weak-areas` | 統計 |

## データモデル

```
subjects (categories) 1 ──< N workbooks (question_sets)
workbooks N >──< N tags        (via question_set_tags)
workbooks 1 ──< N questions
questions N >──< N tags        (via question_tags)
questions 1 ──< N choices

exam_sessions   N >── 1 workbooks
